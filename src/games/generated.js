...
function initAsteroidshoot() {
    ...
    for (let i = 0; i < 5; i++) {
asteroids.push({ x: Math.random() * canvas.width, y: Math.random() * 150, width: 20, height: 20, vx: (Math.random() - 0.5) * 2, vy: 2 });
}
 ...
 asteroids.splice(asteroids.indexOf(a), 1); score += 10;
}
        }
...
requestAnimationFrame(gameLoop);
 ...
 player.x = Math.max(0, Math.min(canvas.width - 30, e.clientX - canvas.offsetLeft - 15));
});
...
ball.vy *= -0.8;
 ...
 score++; ball.x = 250; ball.y = 350; ball.vx = 0; ball.vy = 0;
}
...
player.y = canvas.height;
        ...
        for (let i = 0; i < 3; i++) {
ctx.arc(blackhole.x, blackhole.y, blackhole.radius + i * 5, 0, Math.PI * 2);
}
        ctx.stroke();
        ...
        if (playerDist < blackhole.radius + 15) {
gameRunning = false;
}
...
requestAnimationFrame(gameLoop);
    ...
    document.addEventListener('mousemove', (e) => {
 player.x = Math.max(player.size / 2, Math.min(canvas.width - player.size / 2, e.clientX - canvas.offsetLeft)); player.y = Math.max(player.size / 2, Math.min(canvas.height - player.size / 2, e.clientY - canvas.offsetTop));
});
    gameLoop();
}
...
function initBlizzardrun() {
    ...
    for (let i = 0; i < 5; i++) {
obstacles.push({ x: Math.random() * canvas.width, y: i * 80, width: 60, height: 20 });
}
    ...
    function gameLoop() {
        ...
        for (let i = 0; i < 100; i++) {
 ctx.fillStyle = '#ffffff'; ctx.fillRect(Math.random() * canvas.width, Math.random() * canvas.height, 2, 2);
}
 ...
 obstacles.splice(obstacles.indexOf(o), 1); score += 10; obstacles.push({ x: Math.random() * canvas.width, y: -20, width: 60, height: 20 });
}
        }
...
function initBlockblast() {
    ...
    for (let i = 0; i < 20; i++) {
blocks.push({ x: (i % 5) * 100, y: Math.floor(i / 5) * 50, width: 90, height: 40, color: `hsl(${i * 18}, 100%, 50%)` });
}
...
bricks.push({ x: col * 100, y: row * 30, width: 95, height: 25, active: true });
}
}
...
ball.vy *= -1;
    ...
    document.addEventListener('mousemove', (e) => {
 paddle.x = Math.max(0, Math.min(canvas.width - paddle.width, e.clientX - canvas.offsetLeft - paddle.width / 2));
});
    gameLoop();
}
...
for (let col = 0; col < 5; col++) {
bricks.push({ x: col * 100, y: row * 30 + 20, width: 95, height: 25, active: true });
}
}
...
balls.splice(balls.indexOf(b), 1);
            ...
            if (b.x > paddle.x && b.x < paddle.x + paddle.width && b.y > paddle.y && b.y < paddle.y + paddle.height) {
b.vy *= -1;
}
...
function initBubbleshooter() {
    ...
    for (let i = 0; i < 30; i++) {
bubbles.push({ x: Math.random() * canvas.width, y: Math.random() * (canvas.height / 2), radius: 10, color: colors[Math.floor(Math.random() * colors.length)] });
}
